...
namespace Tactile.UI.Geometry
{
    ...
    public class Polygon : Graphic
    {
        ...
        {
            get => startAngle;
            set => SetFieldAndMarkDirty(out startAngle, value);
        }
        
        public float EndAngle
        {
            get => endAngle;
            set => SetFieldAndMarkDirty(out endAngle, value);
        }

        public int Sides
        {
            get => sides;
            set => SetFieldAndMarkDirty(out sides, value);
        }

        public float InnerRadius
        {
            get => innerRadius;
            set => SetFieldAndMarkDirty(out innerRadius, Mathf.Clamp(value, 0f, 1f));
        }

        private void SetFieldAndMarkDirty<T>(out T field, T value)
        {
            field = value;
            SetVerticesDirty();
        }
